...
BATCH_ID_FILES = [
    (1, os.path.join(ROOT, "data", "batch_id_1.txt")),
    (2, os.path.join(ROOT, "data", "batch_id_2.txt")),
]
...
def process_batch(client, batch_id: str, batch_num: int,
    ...
    """Check one batch and merge its results. Returns (added, failed, skipped_incomplete)."""
    ...
    if batch.status != "completed":
        print(f"  Not yet complete — run again later.")
        return 0, 0, True
...
def main():
    ...
    if not present:
        raise FileNotFoundError(
            "No batch ID files found. Expected batch_id_1.txt and/or batch_id_2.txt"
        )
